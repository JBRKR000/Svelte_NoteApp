...
<nav class="bg-gradient-to-r from-black to-gray-800 text-white shadow-lg">
		...
		<div class="hidden md:flex gap-7 items-center">
			{#each mainLinks as link, index}
				<a
					...
					style="opacity: {mainHoveredLinkIndex === null ? 1 : mainHoveredLinkIndex === index ? 1 : 0.3};"
        ...
        <div class="hidden md:flex gap-7 items-center">
                    {#each secondaryLinks as link, index}
                        <a
                            href={link.href}
                            class="navbar-item transition-opacity duration-600"
                            style="opacity: {secondaryHoveredLinkIndex === null ? 1 : secondaryHoveredLinkIndex === index ? 1 : 0.3};"
	...
	{#if isMobileMenuOpen}
		<div class="md:hidden px-6 pb-4 space-y-2" transition:slide={{ duration: 600 }} >
			{#each mainLinks as link}
				<a
					href={link.href}
					class="block px-4 py-2 rounded hover:bg-gray-500 duration-500">
					{link.label}
				</a>
			{/each}
			<a href="/login" class="block px-4 py-2 rounded hover:bg-gray-500 duration-500">Login</a>
			<a href="/register" class="block px-4 py-2 rounded hover:bg-gray-500 duration-500">Register</a>
		</div>
	{/if}
</nav>
